...
private const val CERT_SEND_COMPLETED = 1
private const val CERT_SEND_FAILED = 0
...
class EspIdfRnModule(reactContext: ReactApplicationContext) :
  ...
  private val bleScanListener: BleScanListener = object : BleScanListener {
    ...
    override fun onPeripheralFound(device: BluetoothDevice, scanResult: ScanResult) {
      ...
      if (serviceUuid != null && !bluetoothDevices.containsKey(serviceUuid)) {
        ...
        val params = mapOf("deviceName" to scanRecord!!.deviceName, "serviceUuid" to serviceUuid)
  ...
  fun startBleScan(prefix: String?, p: Promise) {
    ...
    if (!isScanning && reactApplicationContext.checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
      p.resolve(true)
      isScanning = true
  ...
  fun connectDevice(uuid: String, pop: String?, p: Promise) {
    ...
    p.resolve(true)
    val security = if (pop == null) ESPConstants.SecurityType.SECURITY_0 else ESPConstants.SecurityType.SECURITY_1
    val espDevice = provisionManager.createESPDevice(ESPConstants.TransportType.TRANSPORT_BLE, security)
    if (pop != null) {
  ...
  fun startWifiScan(p: Promise) {
    ...
    provisionManager.espDevice.scanNetworks(object : WiFiScanListener {
      override fun onWifiListReceived(wifiList: ArrayList<WiFiAccessPoint>) {
        val params = Arguments.createMap()
        params.putArray("wifiList", Arguments.makeNativeArray(wifiList.map {
  ...
  @ReactMethod
  fun sendCertificate(thingName: String, endpointUrl: String, deviceCert: String, deviceKey: String, p: Promise) {
    if (!hasConnected(p)) {
      return
    }

    var configRequests: ArrayList<CustomConfig.CustomConfigRequest> = arrayListOf()
    // Name and endpoint request
    var cloudIdReq = CustomConfig.CustomConfigRequest.newBuilder()
    var cloudId = CustomConfig.CloudID.newBuilder()
    cloudId.setThingName(thingName)
    cloudId.setEndpointUrl(endpointUrl)
    cloudIdReq.setCloudId(cloudId.build())
    configRequests.add(cloudIdReq.build())
    // Cert
    var deviceCertSplit = deviceCert.chunked(100)
    for (piece in deviceCertSplit) {
        var pieceReq = CustomConfig.CustomConfigRequest.newBuilder()
        var chunk = CustomConfig.CertChunk.newBuilder()
        chunk.setChunk(piece)
        pieceReq.setCertChunk(chunk.build())
        configRequests.add(pieceReq.build())
    }
    // Key
    var deviceKeySplit = deviceKey.chunked(100)
    for (piece in deviceKeySplit) {
      var pieceReq = CustomConfig.CustomConfigRequest.newBuilder()
      var chunk = CustomConfig.KeyChunk.newBuilder()
      chunk.setChunk(piece)
      pieceReq.setKeyChunk(chunk.build())
      configRequests.add(pieceReq.build())
  }

    Log.d(TAG, "Will send chunks with count: " + configRequests.size + " and chunks: " + configRequests)
    sendCertificatePart(configRequests, p)
  }

  fun sendCertificatePart(configRequests: ArrayList<CustomConfig.CustomConfigRequest>, p: Promise) {

    var newConfigRequests: ArrayList<CustomConfig.CustomConfigRequest> = arrayListOf()
    newConfigRequests.addAll(configRequests)
    var configRequest = newConfigRequests.removeFirst()
    Log.d(TAG, "... will send chunk: " + configRequest + " and pending: " + newConfigRequests.size)

    provisionManager.espDevice.sendDataToCustomEndPoint("cloud-config", configRequest.toByteArray(), object : ResponseListener {

      override fun onSuccess(returnData: ByteArray) {
        // If there are more parts, loop
        if (!newConfigRequests.isEmpty()){
          Log.d(TAG, "... ... chunk sent: " + configRequest + " with result: " + String(returnData, Charsets.UTF_8))
          sendCertificatePart(newConfigRequests, p)
          return
        }
        val params = Arguments.createMap()
        params.putInt("status", CERT_SEND_COMPLETED)
        params.putString("message", String(returnData, Charsets.UTF_8))
        sendEvent(EVENT_SEND_CERT, params)
      }

      override fun onFailure(e: Exception) {
        val params = Arguments.createMap()
        params.putInt("status", CERT_SEND_FAILED)
        params.putString("message", e.message)
        sendEvent(EVENT_SEND_CERT, params)
      }
    })
  ...
  private fun requestLocationPermission() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      if (currentActivity != null && currentActivity is PermissionAwareActivity) {
        ...
        Log.e(TAG, "requestLocationPermission: Activity is null or doesn't implement PermissionAwareActivity")
      }
    }
  }
  ...
  override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String?>?, grantResults: IntArray?): Boolean {
    ...
    when (requestCode) {
      REQUEST_FINE_LOCATION -> {
        ...
        val status = grantResults?.size!! > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED
